...
class ArcMetric {
  ...
  private void calcStartEndSeg() {
    mStartEndSegment = (float) Math.sqrt(
        Math.pow(mStartPoint.x - mEndPoint.x, 2) + Math.pow(mStartPoint.y - mEndPoint.y, 2));
  }
  ...
  private void calcDegrees() {
    mZeroStartSegment = (float) Math.sqrt(
        Math.pow(mZeroPoint.x - mStartPoint.x, 2) + Math.pow(mZeroPoint.y - mStartPoint.y, 2));
    mZeroStartDegree = ArcUtils.acos(
        (2 * Math.pow(mRadius, 2) - Math.pow(mZeroStartSegment, 2)) / (2 * Math.pow(mRadius, 2)));
    switch (mSide) {
      case RIGHT:
