...
public class RetrofitFragment extends Fragment {
  ...
  @Override public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container,
    ...
    _adapter = new ArrayAdapter<>(getActivity(), R.layout.item_log, R.id.item_log,
  ...
  public void onListContributorsWithFullUserInfoClicked() {
    ...
    _subscriptions.add(_api.contributors(_username.getText().toString(), _repo.getText().toString())
        ...
        .flatMap(new Func1<Contributor, Observable<Pair<User, Contributor>>>() {
          @Override public Observable<Pair<User, Contributor>> call(Contributor contributor) {
            Observable<User> _userObservable =
                _api.user(contributor.login).filter(new Func1<User, Boolean>() {
                  @Override public Boolean call(User user) {
                    return !isEmpty(user.name) && !isEmpty(user.email);
                  }
                });

            return Observable.zip(_userObservable, Observable.just(contributor),
        ...
        .subscribe(new Observer<Pair<User, Contributor>>() {
          ...
          @Override public void onNext(Pair<User, Contributor> pair) {

            Log.e(TAG, "isUnsubscribed: " + _subscriptions.isUnsubscribed());
